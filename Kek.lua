...
Server_Kek = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_Kek.."Token_DevKek") then
...
redis:set(Server_Kek.."Token_DevKek",token)
end 
else
io.write('\n\27[1;31m⌔︙Token was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua Kek.lua')
end
------------------------------------------------------------------------------------------------------------
if not redis:get(Server_Kek.."User_DevKek1") then
...
redis:set(Server_Kek.."User_DevKek1",User_Info.Info.Username)
redis:set(Server_Kek.."Id_DevKek",User_Info.Info.Id)
else
...
------------------------------------------------------------------------------------------------------------
local DevKek_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DevKek_Info_Sudo:write([[
do 
local Kek_INFO = {
Id_DevKek = ]]..redis:get(Server_Kek.."Id_DevKek")..[[,
UserName_Kek = "]]..redis:get(Server_Kek.."User_DevKek1")..[[",
Token_Bot = "]]..redis:get(Server_Kek.."Token_DevKek")..[["
}
return Kek_INFO
end

]])
DevKek_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_Kek = io.open("Kek", 'w')
Run_File_Kek:write([[
#!/usr/bin/env bash
cd $HOME/Storm
token="]]..redis:get(Server_Kek.."Token_DevKek")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Kek.lua -p PROFILE --bot=$token
done
]])
Run_File_Kek:close()
------------------------------------------------------------------------------------------------------------
local Run_SM = io.open("Ke", 'w')
Run_SM:write([[
#!/usr/bin/env bash
cd $HOME/Storm
while(true) do
...
os.execute('chmod +x Kek')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_Kek.."Token_DevKek");redis:del(Server_Kek.."Id_DevKek");redis:del(Server_Kek.."User_DevKek1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_Kek
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DevKek
Ids_Dev = {sudos.Id_DevKek,959965834,942979469,1106830477,1010918290,665877797,842721206,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "كيك"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
local function Dev_Kek_User(user)  
local Dev_Kek_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_Kek_User = true  
end  
...
if Dev_Kek_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/TeamKek/KeK/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/TeamKek/KeK/master/Kek.lua'
file_path = 'Kek.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
local UserName = (data.username_ or "TeamKeK")
for gmatch in string.gmatch(data.first_name_, "[^%s]+") do
data.first_name_ = gmatch or 'Kek'
end
...
------------------------------------------------------------------------------------------------------------
function FilesKek(msg)
File_Bot = dofile("Script.lua")
if File_Bot.Kek and msg then
Text_File = File_Bot.Kek(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesKekBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.KekFile and msg then
Text_FileBot = Text_FileBot.KekFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_Kek_File(msg,data)
if msg then
msg = data.message_
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DevKek'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DevKek'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DevKek',text)
redis:del(bot_id..'GetTexting:DevKek'..msg.chat_id_..':'..msg.sender_user_id_)
...
local Users = redis:smembers(bot_id.."Kek:UsersBot")
local Groups = redis:smembers(bot_id..'Kek:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'Kek:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:Group"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'Kek:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'Kek:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'Kek:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/TeamKek/KeK/master/Kek.lua','Kek.lua') 
download_to_file('https://raw.githubusercontent.com/TeamKek/KeK/master/Script.lua','Script.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/TeamKek/Files_Kek/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس Kek\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/TeamKek/Files_Kek/master/Files_Kek/"..File_Name)
if Res == 200 then
...
if not Dev_Kek(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:DevKek'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DevKek')
...
if Dev_Kek_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..Kek..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_Kek(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == 'جلب نسخه احتياطيه' and Dev_Kek(msg) or text == 'جلب نسخه الكروبات' and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
if text == ("مسح قائمه العام") and Dev_Kek(msg) or text == ("مسح المحظورين عام") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
elseif text == ("قائمه العام") and Dev_Kek(msg) or text == ("المحظورين عام") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
if Dev_Kek_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^الغاء العام @(.*)$") }, FunctionStatus, nil)
elseif text and text:match("^اضف مطور @(.*)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^اضف مطور @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^حذف مطور @(.*)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
if Dev_Kek_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
if text and text:match("^اضف مطور (%d+)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
elseif text and text:match("^حذف مطور (%d+)$") and Dev_Kek(msg) then
local url,res = http.request('http://teamstorm.tk/chh/?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.Kek == 0 then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @TeamKeK }')   
...
elseif text == "تفعيل المغادره" and Dev_Kek(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_Kek(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_Kek(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_Kek(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_Kek(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_Kek(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_Kek(msg) then
redis:set(bot_id..'GetTexting:DevKek'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_Kek(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_Kek(msg) then 
...
elseif text == ("ردود المطور") and Dev_Kek(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_Kek(msg) then 
...
elseif text == "حذف رد للكل" and Dev_Kek(msg) then 
...
if Dev_Kek_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
redis:srem(bot_id.."President:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id..'Constructor:Group'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Manager:Group'..msg.chat_id_, result.id_)
...
elseif text == "تغير اسم البوت" and Dev_Kek(msg) or text == "تغيير اسم البوت" and Dev_Kek(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_Kek(msg) then 
...
𓂅 .KeK TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 .[Source Channel](t.me/teamkek)

𓂅 .[Source Info](t.me/teamKeK)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 .[TWS KeK](t.me/TeamKeKbot)
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_Kek(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_Kek(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
local UserName = (b.username_ or "TeamKeK")
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_Kek(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevKek = redis:get(bot_id..'Texting:DevKek')
if TextingDevKek then 
send(msg.chat_id_, msg.id_,TextingDevKek)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_Kek(msg) then
redis:del(bot_id..'Texting:DevKek')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_Kek(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_Kek_File(msg,data)
FilesKekBot(msg,data)
